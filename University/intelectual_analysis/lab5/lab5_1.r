...
set.seed(123) 
sample_data <- rpois(n_single, lambda = lambda_true)

cat(paste("Вибіркове середнє:", mean(sample_data)))

log_l <- function(lambda, data) {
  if (lambda <= 0) return(-Inf) 
  n <- length(data)
  sum_x <- sum(data)
  
  ll <- sum_x * log(lambda) - n * lambda - sum(log(factorial(data)))
  return(ll)
}

lambda_range <- seq(0.01, lambda_true * 2, by = 0.01)
ll_values <- sapply(lambda_range, log_l, data = sample_data)

# Знаходимо теоретичне ОМП (вибіркове середнє)
lambda_mle_single <- mean(sample_data)

png("Graphic\\5_1lab_Log_Likelihood.png", width = 800, height = 600)
...
text(lambda_mle_single, max(ll_values) - 5, 
     labels = paste("ОМП =", lambda_mle_single), 
     col = "red", pos = 4)
dev.off()
...
for (i in 1:length(sample_sizes)) {
  n <- sample_sizes[i]
  current_sample <- rpois(n, lambda = lambda_true)
  mle_estimates[i] <- mean(current_sample)
  cat(paste("n =", n, ": ОМП =", mle_estimates[i], "\n"))
}
...
png("Graphic\\5_1lab_MLE_vs_SampleSize.png", width = 800, height = 600)
